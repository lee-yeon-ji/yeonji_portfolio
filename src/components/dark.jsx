...
export default function Header() {
    ...
    return (
        <div className="sticky top-0 bg-white dark:bg-gray-800 position-fixed w-full left-0 rigth-0 z-999 transition-colors duration-300">
            <header className="max-w-7xl mx-auto bg-white dark:bg-gray-800">
                <div className="flex justify-between items-center w-full h-16 font-bold text-2xl px-4 text-gray-900 dark:text-white">
                    ...
                    <nav className="items-center space-x-6 hidden sm:flex">
                        {navItems.map(item => (
                            <NavLink key={item.id} to={item.to} className="hover:text-gray-400 dark:hover:text-gray-300">
                                {item.label}
                            </NavLink>
                ...
                {/* mobile Menu */}
                <aside className={`
            fixed top-0 right-0 w-64 h-full bg-gray-300 dark:bg-gray-700 z-999
